<template>
    <div style="padding: 20px;">
        <el-row>
            <h3 style="margin-left: 2%;margin-top: 0;">门店可调项-CAD规则</h3>
        </el-row>
        <el-row class="toolbar search-toolbar">
            <el-col :span="24">
                <el-form :inline="true" :model="searchDto"  class="form-search" label-width="150px">
                    ...
                    <el-form-item label="cad部位:" style="margin-bottom: 10px;">
                        <el-select v-model="searchDto.cadPositionId" placeholder="请选择">
                            <el-option
                                v-for="item in cadPositionList"
                                :key="item.id"
                                :label="item.name"
                                :value="item.id">
                            </el-option>
        ...
        <el-dialog
            ...
            <el-form :model="submitFormData" label-width="25%" ref="submitFormData">
                ...
                <el-form-item label="CAD部位:" align="left">
                    <el-select v-model="submitFormData.cadPositionId" placeholder="请选择">
                        <el-option
                            v-for="item in cadPositionList"
                            :key="item.id"
                            :label="item.name"
                ...
                <el-form-item label="CAD调整值" align="left">
                    <el-input-number v-model="submitFormData.adjustValue" ></el-input-number>
                </el-form-item>
                <el-form-item label="CAD计量单位" align="left">
...
<script>
    import {storeAdjustmentItemFind,ruleCadAdd,ruleCadEdit,ruleCadList,cadPositionFind,fittingBagFind,fittingBagList} from '../../api/api';
    export default {
        data() {
            return {
                searchDto:{
        ...
        methods:{
            ...
            search(){
                var params = {
                    ...
                    'cadPositionId':this.searchDto.cadPositionId,
					'rows':this.pageSize,
					'page':this.currentPage
                };
				ruleCadList(params).then(res => {
					if('200' == res.returnCode) {
            ...
            initCadPositionList(){
                ...
                cadPositionFind(params).then(res => {
					if('200' == res.returnCode) {
                        if(res.returnData && res.returnData.length > 0) {
                            this.cadPositionList = res.returnData;
                        }
                    }
				}) .catch(err => {
					console.log(err);
            ...
            initSubmitFormData(){
	            ...
	            this.submitFormData.cadPositionId = "";
            ...
            edit(row) {
                ...
                if(row) {
                    ...
                    this.submitFormData.cadPositionId = row.cadPosition.id;
            ...
            submitForm() {
                ...
                if(!this.submitFormData.cadPositionId) {
                    this.$message({message: "请选择CAD部位",type: 'error'});
                    return;
                }
                if(!this.submitFormData.adjustValue) {
                    this.$message({message: "请输入CAD调整值",type: 'error'});
                    return;
                }
                if(!this.submitFormData.unit) {
                    this.$message({message: "请输入CAD计量单位",type: 'error'});
                    return;
                }
                var param = {
	                ...
	                cadPositionId:this.submitFormData.cadPositionId,
                ...
                this.hiddenDialog();
                var func = param.id?ruleCadEdit(param):ruleCadAdd(param);
                func.then(res => {
                    if ('200' == res.returnCode) {
