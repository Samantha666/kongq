<template>
  <div style="padding: 20px;">
    <el-row>
      <h3 style="margin-left: 2%;margin-top: 0;">质检部位管理</h3>
    </el-row>
    <!-- 搜索框 -->
    ...
    <!-- 新增模态框 -->
    <el-dialog title="新增质检部位" :visible.sync="addDialogVisible"
...
<script>
  ...
  import {qualityCheckingPositionList,qualityCheckingPositionSaveOrUpdate,qualityCheckingPositionDel} from '../../api/api';
  export default {
    // components: {
    //   ElTable,
    //   ElOption,
    ...
    methods: {
          ...
          self.$refs[formName].validate((valid) => {
            if (valid) {
              self.$confirm('确认新增质检部位?', '提示', {
                ...
                // console.log(params);
                qualityCheckingPositionSaveOrUpdate(params).then(function (res) {
                  if(res.returnCode === '200'){
      ...
      handleClickDel(row) {
        ...
        this.$confirm('确认删除此条质检数部位?', '提示', {
          ...
          qualityCheckingPositionDel(params).then(function (res) {
            if(res.returnCode === '200'){
