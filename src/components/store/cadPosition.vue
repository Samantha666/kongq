<template>
  <div style="padding: 20px;">
    <el-row>
      <h3 style="margin-left: 2%;margin-top: 0;">cad部位管理</h3>
    </el-row>
    <!-- 搜索框 -->
    ...
    <!-- 新增模态框 -->
    <el-dialog title="新增cad部位" :visible.sync="addDialogVisible"
...
<script>
  ...
  import {cadPositionList,cadPositionSaveOrUpdate,cadPositionDel} from '../../api/api';
  export default {
    // components: {
    //   ElTable,
    //   ElOption,
    ...
    methods: {
        ...
        self.$refs[formName].validate((valid) => {
          if (valid) {
            self.$confirm('确认新增cad部位?', '提示', {
              ...
              // console.log(params);
              cadPositionSaveOrUpdate(params).then(function (res) {
                if(res.returnCode === '200'){
      ...
      handleClickDel(row) {
        ...
        this.$confirm('确认删除此条cad部位?', '提示', {
          ...
          cadPositionDel(params).then(function (res) {
            if(res.returnCode === '200'){
