<template>
  <div style="padding: 20px;">
    <el-row>
      <h3 style="margin-left: 2%;margin-top: 0;">订单bom管理</h3>
    </el-row>
    <!-- 搜索框 -->
    <!-- 搜索框 -->
    ...
    <!-- 表格 -->
    <!-- 门店列表 -->
    <el-table :data="tableData" highlight-current-row  style="width: 100%" @row-click="clickRow">
...
<script>
  ...
  import {returnDownloadOrderData,CraftsStoreList,fittingBagList,styleList,styleColorListByStyleId,sizeList,cadPositionList,storeAdjustmentBomCheckingSaveOrUpdate,bomPositionFind,materialList} from '../../api/api';
  import ElInput from "element-ui/packages/input/src/input";
  export default {
    // components: {
    //   ElTable,
    ...
    methods: {
        ...
        CraftsStoreList(params).then(function (res) {
          if(res.returnCode == '200'){
            ...
            for(var j=0;j<self.tableData.length;j++){
              self.StoreID = self.tableData[j].id;
            }
      ...
      handleClickAdd(row) {
        ...
        this.rowData = row.bomList;
        if(this.rowData.length == 0){
          this.addFormData.remark ='';
          this.addFormData.id = row.id;
          this.addFormData.operatorId = this.userInfo.accountId
        }else{
          this.addFormData.remark = row.bomList[0].remark;
          this.addFormData.id = row.id;
          this.addFormData.operatorId = this.userInfo.accountId
        }
            ...
            self.$confirm('确认新增订单Bom?', '提示', {
              ...
              if(self.rowData.length == 0){
                ...
                var params = {
                  ...
                  operatorId: self.addFormData.operatorId,
                };
              ...
              storeAdjustmentBomCheckingSaveOrUpdate(params).then(function (res) {
                //console.log(res)
                if(res.returnCode === '200'){
      ...
      clickRow(row) {
        ...
        console.log(row);
        this.checkRow = row;
        if(row.bomList.length == 0){
          this.$message.warning('当前订单没有bom清单，请添加')
          this.tableMaterialData = []
        }else{
          if(row.bomList[0].bom == '[]'){
            this.$message.warning('当前订单没有bom清单，请添加')
            this.tableMaterialData = []
          }else{
            this.tableMaterialData = JSON.parse(row.bomList[0].bom);
          }
        }

        //console.log(this.tableMaterialData)
        for(var i=0;i<this.tableMaterialData.length;i++){
            if(this.tableMaterialData[i].status == '1'){
              this.tableMaterialData[i].statusName = '生效'
            }else if(this.tableMaterialData[i].status == '2'){
              this.tableMaterialData[i].statusName = '失效'
            }else{
              this.tableMaterialData[i].statusName = '状态错误'
            }
        }
        //alert(1111)
        //console.log(this.tableMaterialData)
...
//          /var delData = []
          ...
          var params = {
            ...
            operatorId:this.userInfo.accountId
          };
          //console.log(this.tableMaterialData)
          var self = this
          storeAdjustmentBomCheckingSaveOrUpdate(params).then(function (res) {
